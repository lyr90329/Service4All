...
import cn.edu.buaa.act.service4all.bpmndeployment.message.BPMNDocQueryMsgFactory;
import cn.edu.buaa.act.service4all.bpmndeployment.message.BPMNDocQueryMsgParser;
...
public class BPMNDocQueryReceiver extends BPMNReceiver {
	
	private Log logger = LogFactory.getLog(BPMNDocQueryReceiver.class);
	
	protected void initParser(AppEngineContext context){
		this.parser = new BPMNDocQueryMsgParser();
	}
	
	protected void initFactory(AppEngineContext context){
		//init the factory
		this.factory = new BPMNDocQueryMsgFactory();
	}
	...
	public Document createResponseDocument(ExchangeContext context)
		...
		logger.info("Create the response message document for service document query");
	...
	public void handlRequest(Document request, ExchangeContext context)
		...
		if(parser.validate(request)){
			...
			if(t instanceof ServiceQueryTask){
				
				ServiceQueryTask task = (ServiceQueryTask)t;
				context.storeData(BPMNBusinessUnit.TASK, task);
				this.unit.dispatch(context);
				
			}else{
				//the created task is not ServiceDeployTask
				logger.warn("The created task is not type of ServiceQueryTask");
				//throw new MessageExchangeInvocationException("The created task is not type of ServiceDeployTask");
				BPMNDeploymentException e = new BPMNDeploymentException("The created task is not type of ServiceQueryTask");
				t.setException(e);
			...
			Document excepDoc = factory.createExceptionDocument("The message for service query is invalidate");
			this.sendResponseMessage(excepDoc, context);
		}
	}
}
